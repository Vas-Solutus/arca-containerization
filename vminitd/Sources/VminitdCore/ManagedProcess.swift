...
extension ManagedProcess {
    func start() async throws -> Int32 {
        do {
            ...
            // Wait for the errorPipe to close (after exec), surfacing anything vmexec wrote
            // to it. Upstream replaced the previous blind `syncPipe.readToEnd()` wait with
            // this, so an exec failure reports its cause instead of vanishing. Adopted here
            // in the fork's out-of-lock phase rather than upstream's in-lock position. Safe:
            // the parent's write end of errorPipe is closed in phase 1 above, so readToEnd()
            // terminates rather than blocking on a still-open writer.
