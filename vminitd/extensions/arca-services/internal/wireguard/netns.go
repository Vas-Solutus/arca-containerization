...
// createContainerNetNs creates a new network namespace for a container
// The namespace is created as a named netns at /run/netns/arca-<containerID>
// This allows the container to join an already-configured namespace
// Returns the path to the namespace file
func createContainerNetNs(containerID string) (string, error) {
	log.Printf("Creating network namespace for container %s...", containerID)

	// CRITICAL: Lock goroutine to OS thread for namespace operations
	runtime.LockOSThread()
	defer runtime.UnlockOSThread()

	// Create /run/netns if it doesn't exist (standard location for named namespaces)
	// Note: Use /run/netns (not /var/run/netns) because /run is mounted in the VM but /var/run is not
	netnsDir := "/run/netns"
	if err := os.MkdirAll(netnsDir, 0755); err != nil {
		return "", fmt.Errorf("failed to create netns directory: %w", err)
	}

	// Use a short name to avoid path length issues
	// Take first 12 chars of container ID (like Docker short IDs)
	shortID := containerID
	if len(shortID) > 12 {
		shortID = shortID[:12]
	}
	nsName := fmt.Sprintf("arca-%s", shortID)
	nsPath := filepath.Join(netnsDir, nsName)

	// Check if namespace already exists
	if _, err := os.Stat(nsPath); err == nil {
		log.Printf("Namespace %s already exists, reusing", nsPath)
		return nsPath, nil
	}

	// Save current namespace to return to
	...
	newNs, err := netns.New()
	if err != nil {
		return "", fmt.Errorf("failed to create new namespace: %w", err)
	}
	defer newNs.Close()

	// Bring up loopback in the new namespace (required for 127.0.0.1 to work)
	...
	log.Printf("Loopback interface up in new namespace")

	// Return to original namespace before creating the bind mount
	if err := netns.Set(origNs); err != nil {
		return "", fmt.Errorf("failed to return to original namespace: %w", err)
	}
	...
	// /proc/self/fd/<fd> gives us a path to the namespace
	nsFd := int(newNs)
	nsSourcePath := fmt.Sprintf("/proc/self/fd/%d", nsFd)

	if err := unix.Mount(nsSourcePath, nsPath, "", unix.MS_BIND, ""); err != nil {
		os.Remove(nsPath)
		return "", fmt.Errorf("failed to bind mount namespace: %w", err)
	}

	log.Printf("Created and persisted network namespace: %s", nsPath)
	return nsPath, nil
}
